...
def motion_global_anchor_position_error_exp(env: EnvTypes, env_states: TensorState, cmd: MotionCommand, std: float) -> torch.Tensor:
    robot_state = env_states.robots[env.name]
    error = torch.sum(torch.square(cmd.anchor_pos_w - robot_state.root_state[:, :3]), dim=-1)
    return torch.exp(-error / std**2)


def motion_global_anchor_orientation_error_exp(env: EnvTypes, env_states: TensorState, cmd: MotionCommand, std: float) -> torch.Tensor:
    robot_state = env_states.robots[env.name]
    error = quat_error_magnitude(cmd.anchor_quat_w, robot_state.root_state[:, 3:7]) ** 2
    return torch.exp(-error / std**2)


def motion_relative_body_position_error_exp(
    env: EnvTypes, env_states: TensorState, cmd: MotionCommand, std: float, body_names: list[str] | None = None
) -> torch.Tensor:
    robot_state = env_states.robots[env.name]
    body_indexes = get_indexes(env, body_names, env_states.robots[env.name].body_names)
    error = torch.sum(
        torch.square(cmd.body_pos_relative_w[:, body_indexes] - robot_state.body_state[:, body_indexes, :3]), dim=-1
    )
...
def motion_relative_body_orientation_error_exp(
    ...
    body_indexes = get_indexes(env, body_names, env_states.robots[env.name].body_names)
    error = (
        quat_error_magnitude(cmd.body_quat_relative_w[:, body_indexes], robot_state.body_state[:, body_indexes, 3:7])
        ** 2
    )
    return torch.exp(-error.mean(-1) / std**2)
...
def motion_global_body_linear_velocity_error_exp(
    ...
    body_indexes = get_indexes(env, body_names, env_states.robots[env.name].body_names)
    error = torch.sum(
        torch.square(cmd.body_lin_vel_w[:, body_indexes] - robot_state.body_state[:, body_indexes, 7:10]), dim=-1
    )
    return torch.exp(-error.mean(-1) / std**2)
...
def motion_global_body_angular_velocity_error_exp(
    ...
    body_indexes = get_indexes(env, body_names, env_states.robots[env.name].body_names)
    error = torch.sum(
        torch.square(cmd.body_ang_vel_w[:, body_indexes] - robot_state.body_state[:, body_indexes, 10:13]), dim=-1
    )
    return torch.exp(-error.mean(-1) / std**2)
...
def action_rate_l2(env: EnvTypes, env_states: TensorState, **kwargs) -> torch.Tensor:  # TODO check where action is stored
...
def joint_pos_limits(env: EnvTypes, env_states: TensorState, **kwargs) -> torch.Tensor:
